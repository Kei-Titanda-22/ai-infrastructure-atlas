...
- [x] v0.4 KOKUSAI ELECTRIC / Canon / Nikon equipment history batch deployed — Run #163
- [x] v0.4 Besi / ASMPT equipment-packaging history batch deployed — Run #165
- [x] v0.4 Lasertec annual history deployed — Run #167
- [x] v0.4 HANMI Semiconductor annual history deployed — Run #168
- [x] v0.4 Corning / TE Connectivity / Equinix / Digital Realty history batch deployed — Run #169
- [x] v0.4 Tesla / Mobileye Physical AI history batch deployed — Run #171
- [x] v0.4 Aptiv Physical AI history extension deployed — Run #172
...
時系列財務は `src/data/financial-history.json` と `financial-history-v04-batch2.json` 〜 `batch26.json`、監査済みcash-flow overrideを `src/lib/financial-history.ts` で統合した配列を正規化履歴として扱う。各期間に四半期/通期、期末日、通貨・単位、会計基準、一次資料、検証日を持たせ、売上高・営業利益・営業利益率・FCF・設備投資を `value / status / basis` で管理する。

現在の履歴対象は76社・199期間で、76社すべてが2期間以上を持つ。直近Run #145〜#172では、OSAT / パッケージ基板、半導体材料、電力インフラ、データセンターHVAC、EDA、電力半導体、半導体装置、インターコネクト / データセンター、Physical AIを重点的に拡張した。
...
Run #163ではKOKUSAI ELECTRIC、Canon、Nikonを各2通期追加した。3社はいずれも新規履歴対象で、6期間すべて売上高・営業利益・営業利益率・営業CF・cash Capexを一次資料で収録する。

- KOKUSAI ELECTRICはadjusted operating profitではなくIFRS reported operating profitを使用し、Atlas CapexをPP&E＋intangible asset purchasesとする。
- CanonはUS GAAP cash-flow statementのfixed-asset cash purchasesを使用し、発生ベースcapital expendituresと区別する。会社FCFのtotal investing CF定義は流用しない。
- NikonはIFRS reported operating profit/lossを使用し、資産・事業売却等を含む会社FCFを流用しない。FY2026はAtlas FCFが-60,177百万円となり、会社FCFとの差異をbasisに明示する。

Run #165ではBesiのQ4 2024 / Q4 2025とASMPTのFY2024 / FY2025を追加した。Besiは2期間とも売上高・IFRS営業利益・営業利益率・営業CF・設備投資を収録し、Atlas FCFを `Q4営業CF − tangible capex − capitalized R&D` とする。ASMPTはNEXXが非継続事業へ再分類されたため、継続事業ベースの売上高・HKFRS営業利益・営業利益率だけを収録し、全社CF表から継続事業のFCF / Capexを按分・推定しない。

Run #167ではLasertecのFY2024 / FY2025を追加した。FY2025有価証券報告書と比較列を使用し、Japanese GAAPのreported operating incomeを採用する。Atlas FCFは `営業活動CF − 有形固定資産取得 − 無形固定資産取得` とし、会社独自の投資CFベース指標へ置換しない。

Run #168ではHANMI SemiconductorのFY2024 / FY2025を追加した。Korea Exchange KIND掲載のFY2025事業報告書と比較列からK-IFRS連結売上高・営業利益・営業CF・PP&E取得・無形資産取得を収録し、元のKRW値はmillionへ単位スケールのみ変換する。Atlas FCFは `営業活動CF − PP&E取得 − 無形資産取得` とする。

### Interconnect / Data Center

Run #169ではCorning、TE Connectivity、Equinix、Digital RealtyをFY2024 / FY2025の2通期ずつ追加した。4社ともSEC Form 10-KのUS GAAP連結値を使用し、売上高・reported operating income・営業利益率を一次資料で正規化する。

- CorningはGAAP営業CFからcash-flow statementのcapital expendituresを控除してAtlas FCFを算出し、adjusted operating cash flowベースの会社指標を流用しない。
- TE Connectivityはadjusted operating incomeではなくGAAP operating incomeを採用し、Atlas FCFを `GAAP operating cash flow − capital expenditures` とする。
- EquinixはデータセンターREITの開発・拡張投資を既存の産業会社向けcash-Capexへ強制的に正規化せず、FCF / Capexを未収録とする。
- Digital RealtyもREITの不動産開発・改良・取得・売却を産業会社のcash-Capexと同一視せず、FCF / Capexを未収録とする。

### Physical AI

Run #171ではTeslaとMobileyeをFY2024 / FY2025の2通期ずつ追加し、Run #172ではAptivを同じbatch26へ追加した。3社とも一次資料のreported GAAP値を使用し、調整後営業利益へ置換しない。

- TeslaはUS GAAPの`income from operations`を採用し、Atlas FCFを `GAAP operating cash flow − purchases of property and equipment excluding finance leases, net of sales` とする。
- Mobileyeは2024年の2,695百万ドルgoodwill impairmentを含むGAAP operating lossをそのまま採用する。Atlas Capexはcash purchase of property and equipmentのみとし、non-cash PP&E purchaseを除外する。
- AptivはUS GAAP consolidated operating incomeを採用し、FY2025の648百万ドルgoodwill impairmentを含める。Atlas FCFは `GAAP operating cash flow − capital expenditures` とする。

Run #172で199期間 / 76社 / 889検証済み指標 / FCF+Capex 147期間を確認した。
...
`/financials/` は76社を企業切替対象とし、四半期・通期を分離した自前SVG推移図と、一次資料・会計基準・検証状態付き決算表を表示する。FCF / Capexが存在する企業では追加グラフを表示し、負のFCFにも対応する。決算グラフはPR #32の大型化をロールバック済みで、PR #32直前の表示状態を維持する。
...
`/financials/updates/` は正規化履歴から直接生成する決算更新台帳。現在199レコード / 76社を、1行「1企業 × 1決算期間 × 1一次資料」として表示する。企業比較画面はv0.4正規化履歴を消費し、v0.3の `?ids=` 比較URL契約を維持する。

`scripts/validate-v04.py` は `financial-history-v04-batch*.json`、v0.4 Document Source、Source Policyを自動検出する。Source/company対応、ISO期末日、会社・期間重複、5指標スキーマ、欠損理由、verifiedAt、営業利益率再計算、Capex符号、Atlas算出FCFの入力値・算式、Source/Policy対応、主要企業ごとの最低収録数を検査する。Run #172以降の回帰下限は199期間 / 76社 / 889検証済み指標 / FCF+Capex 147期間 / Source 81件で、Besi / ASMPT / Lasertec / HANMI Semiconductor / Corning / TE Connectivity / Equinix / Digital Realty / Tesla / Mobileye / Aptivも各2期間以上を個別ゲートで保持する。

Run #172では `v0.4 financial-history validation OK: 199 periods / 76 companies / 76 multi-period companies / 889 verified metrics / 147 FCF+Capex periods / 5 cash-flow overrides / 81 v0.4 document sources+policies` を確認した。Astroは**109ページ**、Pagefindは**105ページ / 2,905語**を生成し、GitHub Pages deployまで成功した。
...
4. Physical AI / 産業機器ではDENSO / FANUC / Yaskawa / Omron / Keyence / SMC / Mitsubishi Electric等を一次資料の閉じやすさ順に複数期間化する
5. 既存76社について必要に応じて四半期の連続性をさらに伸ばす
