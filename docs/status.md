...
- normalized financial history: **94 periods / 29 companies**
- multi-period financial-history companies: **29 / 29 covered companies**
- verified normalized historical metrics: **418**
- periods with both FCF and Capex: **68**
- audited cash-flow overrides: **5**
- v0.4 exact document sources: **28**
- v0.4 pending source policies: **28**
- earnings update ledger: **94 normalized records / 29 companies**
...
時系列財務は `src/data/financial-history.json` と `financial-history-v04-batch2.json` 〜 `batch9.json`、監査済みcash-flow overrideを `src/lib/financial-history.ts` で統合した配列を正規化履歴として扱う。各期間に四半期/通期、期末日、通貨・単位、会計基準、一次資料、検証日を持たせ、売上高・営業利益・営業利益率・FCF・設備投資を `value / status / basis` で管理する。

現在の履歴対象は29社・94期間で、29社すべてが2期間以上を持つ。直近batchではGlobalFoundries、UMC、Texas Instruments、Analog Devices、NXPを各2期間追加した。5社10期間すべてで売上高・営業利益・営業利益率・FCF・Capexを一次資料に戻って検証した。
...
Western DigitalはQ4 FY2025 / Q3 FY2026 / Q4 FY2026 / FY2025 / FY2026の5期間を保持する。FY2025の会社開示FCFにはpre-Separation Flash Ventures活動148百万ドルが含まれるため、その会社固有調整を除外し、Atlas統一定義 `営業活動CF − PP&E取得支出` で1,284百万ドルを保持する。
...
東京エレクトロンもFY2026 Q1 / Q2 / Q3 / Q4とFY2027 Q1の5四半期、FY2025 / FY2026の2通期を収録する。Q2〜Q4は累計値の差分から単四半期化した。TELのCapex定義は既存FY2027 Q1と揃え、`有形固定資産の取得による現金支出` のみを採用し、FCFは `営業活動CF − 同Capex` とする。
...
`/financials/` は29社を企業切替対象とし、四半期・通期を分離した自前SVG推移図と、一次資料・会計基準・検証状態付き決算表を表示する。FCF / Capexが存在する企業では追加グラフを表示し、負のFCFにも対応する。期間ラベルは12pxの本文色・太字へ変更済み。

全体マップの工程別縦カラーラインは4pxから6pxへ太くし、工程色の彩度を一段引き上げた。情報密度や研究DB調の構造は変更せず、工程の視覚的な識別だけを強めている。

`/financials/updates/` は正規化履歴から直接生成する決算更新台帳。現在94レコード / 29社を、1行「1企業 × 1決算期間 × 1一次資料」として表示する。検証日、企業、期間、検証済み指標、一次資料、状態を持ち、企業と四半期/通期で絞り込みできる。

履歴収録済み29社の個社ページでは財務セクションから決算履歴へ直接移動できる。企業比較画面もv0.4正規化履歴を消費し、v0.3の `?ids=` 比較URL契約を維持したまま、最新収録期の売上高・営業利益・営業利益率・FCF・設備投資を表示する。金額の報告通貨・単位、四半期/通期、FCF/Capex basisが揃わない場合は比較不能または条件注意を明示する。

`scripts/validate-v04.py` はbase / batch2〜batch9 / cash-flow overrideを結合し、Source/company対応、ISO期末日、会社・期間重複、5指標スキーマ、欠損理由、verifiedAt、営業利益率再計算、Capex符号、Atlas算出FCFの入力値・算式、v0.4文書Source/Policy対応を検査する。主要企業ごとの最低収録数も個別ゲートで保持する。

Run #138では `v0.4 financial-history validation OK: 94 periods / 29 companies / 29 multi-period companies / 418 verified metrics / 68 FCF+Capex periods / 5 cash-flow overrides / 28 v0.4 document sources+policies` を確認した。Astroは**109ページ**、Pagefindは**105ページ / 2,736語**を生成し、GitHub Pages deployまで成功した。

## Remaining v0.4 work

1. 正規化履歴カバレッジを現在の29社から100社DB内の次の主要企業群へ拡張する
2. 光通信・データセンター電力・OSAT/基板・半導体材料企業を優先して追加する
3. 既存29社について必要に応じて四半期の連続性をさらに伸ばす
