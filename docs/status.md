...
- verified normalized historical metrics: **156**
- periods with both FCF and Capex: **21**
...
FCF / Capexは定義確認済み期間だけ実データ化する。NVIDIA通期3年、ASML 5四半期、Micron Q3 FY2026に加え、Lam Research、KLA、Applied Materials、Broadcomでは一次資料の営業活動CFと設備投資支出を保持し、Atlas側でFCFを再計算する。Advantest・DISCOは今回参照した比較資料でFCF/Capexを正規化していないため、未収録のまま維持する。
...
`scripts/validate-v04.py` は複数履歴バッチを結合したうえで、Source/company対応、ISO期末日、会社・期間重複、5指標スキーマ、欠損理由、verifiedAt、営業利益率再計算、Capex符号、Atlas算出FCFの入力値・算式、既存監査済み企業の移行漏れを検査する。現在の回帰下限は38期間 / 13社 / 複数期間企業11社 / 156検証済み指標 / FCF+Capex 21期間。

Run #118では `v0.4 financial-history validation OK: 38 periods / 13 companies / 11 multi-period companies / 156 verified metrics / 21 FCF+Capex periods` を確認した。Astroは108ページ、Pagefindは104ページ / 2,627語を生成し、GitHub Pagesは `Reported success!`。配布artifactでもfinancials/compareへ新6社が含まれ、Lam Research・Broadcom等の個社ページに履歴導線が存在することを確認した。比較ページの生成済みインラインJavaScript 3本も `node --check` を通過した。

## Remaining v0.4 work

1. TSMC・キオクシア・東京エレクトロン等のFCF / Capexを一次資料定義付きで追加する
2. Compute / Memory / Network / Data Center側の主要企業へ時系列履歴を拡張する
