...
if len(history) < 177:
    errors.append(f'v0.4 history regression: expected at least 177 periods, got {len(history)}')
if len(covered_companies) < 65:
    errors.append(f'v0.4 coverage regression: expected at least 65 companies, got {len(covered_companies)}')
if len(multi_period_companies) < 65:
    errors.append(f'v0.4 history regression: expected all 65 covered companies to be multi-period, got {len(multi_period_companies)}')
...
if verified_metrics < 791:
    errors.append(f'v0.4 history regression: expected at least 791 verified metrics, got {verified_metrics}')
if cashflow_periods < 131:
    errors.append(f'v0.4 cash-flow regression: expected at least 131 FCF/Capex periods, got {cashflow_periods}')
if len(v04_sources) < 68:
    errors.append(f'v0.4 source regression: expected at least 68 document sources+policies, got {len(v04_sources)}')
