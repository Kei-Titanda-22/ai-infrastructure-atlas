...
source_ids = {s['id'] for s in [*sources, *document_sources, *facility_sources]}
errors = []
...
print(f'Audit/evidence validation OK: {len(audits)} verified metric audits / {len(claims)} evidence claims / {len(value_chain)} value-chain stages / {len(update_log)} update-log entries')
